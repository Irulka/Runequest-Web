 ...
 * 
 * Características:
 * - Buscador al 50% de ancho bajo los botones
 * - Compatible con futuros JSON de otras categorías
 * - Búsqueda inteligente en todas las categorías
 * - Integración completa con tools.json e instrumentos.json
 */
...
async function cargarDatosCategoria(categoria) {
    try {
        // Determinar qué archivo JSON cargar según la categoría
        let archivo = '';
        ...
        else return []; // Para otras categorías aún no implementadas
...
function configurarBotones() {
    Object.keys(categorias).forEach(key => {
        ...
        if (UI.botones[key]) {
            UI.botones[key].addEventListener('click', async () => {
                // Si la categoría no tiene datos cargados, cargarlos
                if (categorias[key].datos.length === 0) {
                    categorias[key].datos = await cargarDatosCategoria(key);
                }
                UI.buscador.value = '';
                mostrarItems(key);
            });
...
async function init() {
    // Cargar datos iniciales (solo herramientas e instrumentos por ahora)
